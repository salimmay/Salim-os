...
const COLORS = {
  ...
  grid: 'rgba(30, 41, 59, 0.5)', // Subtle grid color
  web: 'rgba(148, 163, 184, 0.15)' // Faint connection line
};
...
class SingularityCore {
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    [0, Math.PI/3, -Math.PI/3].forEach(rot => {
        ...
        ctx.rotate(-(rot + this.angle)); // reset
    });
    ctx.restore();
  }
}

// --- 2. STARS (With Connections) ---
class Star {
...
class Rocket {
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    ctx.fillStyle = '#cbd5e1'; ctx.beginPath(); ctx.ellipse(0, 0, 15, 5, 0, 0, Math.PI * 2); ctx.fill(); // Body
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      stars = Array.from({ length: 150 }, () => new Star(canvas.width, canvas.height)); // Increased Star Count
    ...
    const drawGrid = () => {
        ...
        // Floor perspective logic
        const horizon = h * 0.55; // Horizon line
        const step = (time * 0.5) % 50; // Movement
        ...
        // Fading mask for horizon
        const g = ctx.createLinearGradient(0, horizon, 0, h);
        g.addColorStop(0, 'rgba(2, 6, 23, 1)'); // Opaque at horizon
        g.addColorStop(0.2, 'rgba(2, 6, 23, 0)'); // Transparent
        ctx.fillStyle = g;
    ...
    const animate = () => {
      ...
      // 3. STARS & PLEXUS CONNECTIONS
      ctx.lineWidth = 0.3; ctx.strokeStyle = COLORS.web;
      stars.forEach((s, index) => {
          s.update();
          s.draw(ctx);
          // Connect Stars nearby (The Web)
      ...
      // 8. SKILLS (Connect to nearby stars too!) - Updated to use filter
      particles.forEach(p => {
          p.update(ctx, mouseX, mouseY, canvas.width, canvas.height, core.shockwave, filterRef.current);
          // Connect skills to stars for "Integration" feel (only for active/visible particles)
